...
public class GameMap
{
	...
	public void heroMoved(Hero hero) 
	{
		...
		{
			if
			((whoIsOn(hero.getPrevCoord()) == Constants.Ghost  &&  whoIsOn(coord) == Constants.Pacman)
			||(whoIsOn(hero.getPrevCoord()) == Constants.Pacman  &&  whoIsOn(coord) == Constants.Ghost))
				Game.stopGame();
	...
	public boolean lineIsFree(Point p1, Point p2) 
	{ 
		if(p1.x == p2.x) {
			if(p1.y < p2.y) {
